...
function createTsConfigWorkspaceFixture(): { rootDir: string; tsConfigFilePath: string } {
  ...
  writeFileSync(
    join(ignoredDir, 'ignored.ts'), `export const ignored = true`)
...
describe('discovery backend utilities', () => {
  it('builds a compact discovery index for agent-friendly traversal', () => {
    const result = pairDiscoveryWorkspaceScan(createDiscoveryWorkspace([
      { filePath: '/src/contracts.ts', text: 'contracts source' },
      { filePath: '/src/implementations.ts', text: 'implementations source' },
    ]), [
      {
        ...
        contracts: [
          {
            ...
            runtimeName: 'Echo',
            clauses: [],
            clauseSummary: createEmptyScannedContractClauseSummary(),
          },
          {
            kind: 'contract',
            filePath: '/src/contracts.ts',
    ...
    expect(index.contracts.find((contract) => contract.runtimeName === 'Echo')?.implementationIds).toHaveLength(1)
    expect(index.implementations.find((implementation) => implementation.localName === 'unresolved')?.contractId).toBeNull()
  })
  ...
  it('supports workspace discovery backends built around parsed file scanners', () => {
    const workspace = createDiscoveryWorkspace([
      { filePath: '/src/contracts.ts', text: 'contract-file' },
      { filePath: '/src/implementations.ts', text: 'implementation-file' },
    ])
    ...
    const backend = createWorkspaceDiscoveryBackend({ name: 'fake-oxc-ready-backend', scanner })
    const analysis = analyzeDiscoveryWorkspaceWithAstScanner(workspace, scanner)
